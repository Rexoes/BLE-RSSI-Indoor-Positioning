...
//    String str = String(valueFromArduino) + "," + String(zamanBilgisi);
...
//    TX.publish(txBuffer);
 ...
 while ((subscription = mqtt.readSubscription())) {
   if (subscription == &RX) {
     ...
     if(comingValue == "STA1ENABLE"){
       Serial.println("ENB1");
       TX.publish("ESP32 STA1 ENABLED");
     }
     else if(comingValue == "ALLSTAENABLE"){
       Serial.println("ENB1");
       TX.publish("ESP32 STA1 ENABLED");
     }
     else if(comingValue == "STA1DISABLE"){
      TX.publish("ESP32 STA1 DISABLED");
     }
