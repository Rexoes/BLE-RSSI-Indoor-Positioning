...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks
{
    ...
    {
      if(String(advertisedDevice.getName().c_str())=="AP1"){
...
void loop() {
  ...
  if(completeReading == true){
    ...
    //valueFromPC
    if(valueFromPC[0] == 'E' && valueFromPC[1] == 'N' && valueFromPC[2] == 'B'){
      if(valueFromPC[3] == '1'){
        ENABLE = true;
        Serial.println("BLE RSSI SCAN ENABLE");
      }
      else if(valueFromPC[3] == '0'){
        ENABLE = false;
  ...
  if(ENABLE == true){         //BLE RSSI Scan Aktif Edildiyse
    ...
    if(count == 0){
      ...
      Serial.println(String(ESP32_STA_SSID) + "," + String(ESP32_AP_SSID) + "," + String(RSSI_KALMAN));
    }
    else{
      ...
      Serial.println("Toplam Yapilan Olcum Adedi: " + String(count));
